...
- Remaining compiler-register-pin files: **196 / 769 pins**; remaining empty compiler-barrier files: **13 / 13 barriers**
- Maintenance state: **32 legacy inline-asm shims removed** from included-stub files, plus three standalone compiler register pins removed in batch 256; `src/decomp` contains no instruction-bearing inline asm. `func_080EE61C` is now real C: a target-specific `__builtin_swi_div` lowers through the patched agbcc Thumb backend to the BIOS `SVC #6` instruction.
