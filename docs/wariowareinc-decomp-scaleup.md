...
- Verified working tree: `batch 313` — `func_08019ADC` is now an exact strict ordinary-C standalone TU
- `build/report.json`: **1736 / 5914 matched functions** = **29.354074%**
- `matched_code`: **77638 / 993862** = **7.811748%**
- `tools/gen_objdiff.py`: **1276 linked C TUs / 5411 asm-only units** (**6687 total**)
...
- 25% milestone: **1479 / 5914**, exceeded by **257** matched functions.
- 26% milestone: **1538 / 5914**, exceeded by **198** matched functions.
- Active 27% working goal: **1597 / 5914**, exceeded by **139** matched functions.
- Next 30% milestone: **1775 / 5914**; **39** additional matched functions are needed.
...
- Normalized isolation reported only the legacy 30-byte target-symbol boundary versus the complete 36-byte candidate literal-pool section. A Docker full-text proof resolved `dma3_set` as a Thumb symbol at `0x0800100c` and matched both complete sections at SHA-256 `978e31f6b5e6c95d0d1e7567c85667a7c2cafe23eef05e9a25c06a10296764cf`; the narrow force path waived symbol metadata only.
- Fresh Docker report is **1732 / 5914** functions and **77506 / 993862** matched code. Unit coverage is **1272 C / 5415 asm-only**; decomp files are **1473** (**1253 standalone_tu / 220 included_stub**). The function denominator drops by one while total code rises by two because the converted legacy literal-pool boundary is now counted in the report's code/function units. Full strict-audit residue remains **0 instruction-asm files**, **106 files / 502 register pins**, **27 files / 31 barrier findings**, and **7 non-volatile barriers**.
- The clean Docker ROM gate passed with `wariowareinc.gba: OK`; ROM and baseline ROM SHA-1 are both `3f556448d290fa5406d6ed367fee16cc02387ad3`.
...
- Added the canonical `D_083A4B48 = 0x083A4B48` linker assignment already declared in `include/undefined_syms.inc`. Normalized isolation reported only the legacy 26-byte target-symbol boundary versus the complete 32-byte candidate literal-pool section; a Docker full-text proof resolved the external `start_new_task` call as a Thumb symbol at `0x08005790` and matched both complete sections at SHA-256 `2a7c2d01ec0f14356fb13ea593053856d94b05b5c3987fe20ab9f52b0c05a423`.
- Fresh Docker report is **1730 / 5915** functions and **77438 / 993860** matched code. Unit coverage is **1270 C / 5417 asm-only**; decomp files are **1471** (**1251 standalone_tu / 220 included_stub**). The function denominator drops by one while total code rises by two because the converted legacy literal-pool boundary is now counted in the report's code/function units. Full strict-audit residue remains **0 instruction-asm files**, **106 files / 502 register pins**, **27 files / 31 barrier findings**, and **7 non-volatile barriers**.
- The clean Docker ROM gate passed with `wariowareinc.gba: OK`; ROM and baseline ROM SHA-1 are both `3f556448d290fa5406d6ed367fee16cc02387ad3`.
...
- The complete 92-byte linked `.text` section emits the target instruction stream and literal pool byte-for-byte; normalized isolation reports only the legacy target symbol stopping at local label `_08007BA6` (14 bytes versus the complete 92-byte candidate). The narrow force path waived that metadata only, the canonical `D_083A4B10 = 0x083A4B10` linker assignment resolved the table literal, and the integrated clean Docker ROM/report gate matched ROM SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.
- Fresh Docker report is **1727 / 5918** functions and **77326 / 993858** matched code. Unit coverage is **1267 C / 5420 asm-only**; decomp files are **1468** (**1248 standalone_tu / 220 included_stub**). The function denominator drops by one because the converted legacy local-label boundary is no longer inferred as a separate function while total code remains unchanged.
...
- The complete 36-byte linked `.text` section matched the target instruction-for-instruction; normalized isolation reported only the legacy 18-byte target-symbol boundary. The guarded force path waived that metadata only, and the integrated clean Docker ROM/report gate matched ROM SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.
- Fresh Docker report is **1725 / 5920** functions and **77194 / 993856** matched code. Unit coverage is **1265 C / 5422 asm-only**; decomp files are **1466** (**1246 standalone_tu / 220 included_stub**). The denominator drops by one because the converted legacy local-label boundary is no longer counted as a separate inferred function while total units remain unchanged.
...
- Converted `func_08007F48` from `asm/asm_08007f48.s` to a strict ordinary-C standalone TU. Named input, resource, and cache-entry overlays expose the enabled flag, duplicate-key scan, head insertion, size-derived allocation, copied fields, and forward link without instruction asm, barriers, register pins, non-mapped volatile, or opaque layout.
- The candidate's complete 92-byte linked `.text` section matched the target instruction-for-instruction; normalized isolation reported only the legacy 10-byte target-symbol boundary. The guarded force path waived that metadata only, and the integrated clean Docker ROM/report gate matched ROM SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.
- Fresh Docker report is **1724 / 5921** functions and **77158 / 993856** matched code. Unit coverage is **1264 C / 5423 asm-only**; decomp files are **1465** (**1245 standalone_tu / 220 included_stub**). The denominator drops by one because the converted legacy local-label boundary is no longer counted as a separate inferred function while total units remain unchanged.
...
- Converted `func_08007EDC` from `asm/asm_08007edc.s` to a strict ordinary-C standalone TU using the named linked-list node shape shared by `func_08007F20` and `func_08007EAC`. The explicit early-null test and do-while loop preserve predecessor tracking, head/non-head relinking, payload/node deallocation order, and the interworking epilogue without instruction asm, barriers, register pins, non-mapped volatile, or opaque layout.
- The first natural while-loop spelling was a genuine 64-byte near miss: it moved predecessor advancement into the loop preheader and changed the target's branch layout. The accepted v2 spelling preserves the target's complete 68-byte section; isolation then reported only the legacy 14-byte function-symbol boundary. The guarded force path waived metadata only, and the integrated clean Docker ROM/report gate matched ROM SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.
- Fresh Docker report is **1723 / 5922** functions and **77066 / 993856** matched code. Unit coverage is **1263 C / 5424 asm-only**; decomp files are **1464** (**1244 standalone_tu / 220 included_stub**). The denominator drops by one because the converted legacy local-label boundary is no longer counted as a separate inferred function while total units remain unchanged.
...
- Converted `func_08007AD4` from `asm/asm_08007ad4.s` to a strict ordinary-C standalone TU. Separate `result`, `count`, and `value` locals preserve the original destination, unsigned limit check, single source-byte load, byte store, pointer increments, and `u8 *` return. The source contains no instruction asm, barriers, register pins, non-mapped volatile, or opaque offset-heavy layout.
- Round55's earlier candidate for this function had a genuine two-byte `R4`/`R5` prologue-order mismatch and remains historical near-miss evidence. The new v1 spelling preserves the target order; isolation reported only the legacy 10-byte function-symbol boundary versus the complete 36-byte section. The guarded force path waived metadata only, and the integrated clean Docker ROM/report gate matched ROM SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.
- Fresh Docker report is **1722 / 5923** functions and **76998 / 993856** matched code. Unit coverage is **1262 C / 5425 asm-only**; decomp files are **1463** (**1243 standalone_tu / 220 included_stub**). The denominator drops by one because the converted legacy local-label boundary is no longer counted as a separate inferred function while total units remain unchanged.
- Evidence: `.decomp-runs/round-172-isolation.json`, `.decomp-runs/round-172-08007AD4-v1-source-audit.json`, `.decomp-runs/round-172-08007AD4-apply.json`, `.decomp-runs/round-172-full-source-audit.json`, and `.nearmiss/func_08007AD4.json`.
...
- Converted `func_08006CC8` from `asm/asm_08006cc8.s` to a strict ordinary-C standalone TU using the existing typed `gGraphicsBuffer` bitfields. It clears the two target flags without instruction asm, barriers, register pins, non-mapped volatile, or opaque offset-heavy layout.
- The target symbol ended at 24 bytes while the candidate symbol extended to 30 bytes through the literal pool. Complete target/candidate linked `.text` sections were both 32 bytes with equal SHA-256 `416cc334dbb73b34471b3b471474b35bd2428d0ed55b1abc970dfd199272eb4f`; the metadata-only force path and clean Docker ROM gate accepted the conversion.
- Fresh report: **1707 / 5931** functions, **76508 / 993842** matched code, **1247 C / 5440 asm-only** units, and **1448** decomp files (**1228 standalone_tu / 220 included_stub**). Evidence: `.decomp-runs/20260825T153337Z-isolation.json`, `.decomp-runs/round-160-accepted-source-audit.json`, `.decomp-runs/round-160-full-source-audit.json`, `.decomp-runs/round-160-apply.json`, and `.nearmiss/func_08006CC8.json`.

### Batch 288 — one exact ordinary-C standalone linked-list lookup (2026-08-25)
- Converted `func_08007F20` from `asm/asm_08007f20.s` to a strict ordinary-C standalone TU. The function scans the `D_0300485C` linked list, returns the matching node's data address, and otherwise returns the input pointer. The named node model exposes the ID, data, padding, and `next` fields without instruction asm, barriers, register pins, non-mapped volatile, or opaque offset-heavy layout.
- Added the canonical `D_0300485C` standalone linker-map assignment. The normalized linked-ELF comparison saw only the target's 12-byte inferred symbol boundary versus the complete 40-byte body; identical 40-byte linked `.text` sections and SHA-256 justified the metadata-only force path. The clean Docker ROM gate passed with equal ROM/base ROM SHA-1.
- Fresh report: **1706 / 5932** functions, **76476 / 993840** matched code, **1246 C / 5441 asm-only** units, and **1447** decomp files (**1227 standalone_tu / 220 included_stub**). Evidence: `.decomp-runs/20260825T152105Z-isolation.json`, `.decomp-runs/round-159-accepted-source-audit.json`, `.decomp-runs/round-159-full-source-audit.json`, `.decomp-runs/round-159-apply.json`, and `.nearmiss/func_08007F20.json`.
