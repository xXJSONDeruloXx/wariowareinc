...
- m2c supplied the useful scene-field and loop skeletons. asmlift reached project-context compile errors for both candidates; the accepted C uses compiler-only register declarations for exact register ordering, not instruction-bearing or volatile inline asm.
