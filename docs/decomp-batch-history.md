...
- Converted `func_08007B98` from `asm/asm_08007b98.s` to a strict ordinary-C standalone TU. Separate locals retain the original value for the reverse decimal pass, compute the digit count with ordinary unsigned division, and write two-byte entries from `D_083A4B10` in target order. The source has no instruction asm, barriers, register pins, non-mapped volatile, or opaque layout; its two fixed byte-pair accesses pass the strict bounded-layout audit.
- The complete 92-byte linked `.text` section emits the target instruction stream and literal pool byte-for-byte. Normalized isolation reported a misleading near miss because the legacy target symbol inferred only the first 14 bytes through `_08007BA6`; the narrow force path waived that symbol metadata only. Adding `D_083A4B10 = 0x083A4B10` to `undefined_syms.ld` resolved the table literal, and the integrated clean Docker ROM/report gate passed `wariowareinc.gba: OK` with ROM/base SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.
- Report progress advanced **1726 / 5919 → 1727 / 5918** matched functions, **77234 / 993858 → 77326 / 993858** matched code, **1266 / 5421 → 1267 / 5420** linked C/asm-only units, and **1467 → 1468** decomp files. The total-function denominator fell by one because the converted legacy local-label boundary is no longer inferred as a separate function.
...
- Converted `schedule_function_call` from `asm/asm_08007df0.s` to a strict ordinary-C standalone TU with a named three-field task-argument record. The source preserves the `u16` ID normalization, three incoming stack stores, zero outgoing stack argument, canonical `D_083A4B38` task literal, five-argument `start_new_task` call, and live return value without instruction asm, barriers, register pins, non-mapped volatile, or opaque offset-heavy layout.
- The complete 40-byte linked `.text` section matched the target instruction-for-instruction. Normalized isolation reported only the legacy 34-byte target-symbol boundary; the narrow force path waived metadata only, the canonical `D_083A4B38 = 0x083A4B38` linker assignment resolved the ROM literal, and the integrated clean Docker ROM/report gate passed `wariowareinc.gba: OK` with ROM/base SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.
- Report progress advanced **1725 / 5920 → 1726 / 5919** matched functions, **77194 / 993856 → 77234 / 993858** matched code, **1265 / 5422 → 1266 / 5421** linked C/asm-only units, and **1466 → 1467** decomp files. The denominator drops by one while total code rises by two because the converted legacy literal-pool boundary is now counted in the report's code/function units.
...
- Converted `update_scheduled_function_task` from `asm/asm_08007dcc.s` to a strict ordinary-C standalone TU with a named task record. The source preserves the delay decrement/zero return, nullable callback check, indirect callback invocation through the record's argument, and final success return without instruction asm, barriers, register pins, non-mapped volatile, or opaque offset-heavy layout.
- The complete 36-byte linked `.text` section matched the target instruction-for-instruction. Normalized isolation reported only the legacy 18-byte target-symbol boundary; the narrow force path waived metadata only, and the integrated clean Docker ROM/report gate passed `wariowareinc.gba: OK` with ROM/base SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.
- Report progress advanced **1724 / 5921 → 1725 / 5920** matched functions, **77158 / 993856 → 77194 / 993856** matched code, **1264 / 5423 → 1265 / 5422** linked C/asm-only units, and **1465 → 1466** decomp files. The function denominator dropped by one when the converted legacy local-label boundary stopped being inferred as a separate function.
...
## Batch 303 — one exact ordinary-C linked-list cache-entry creator (2026-08-25)
- Converted `func_08007F48` from `asm/asm_08007f48.s` to a strict ordinary-C standalone TU with named input, resource, and cache-entry overlays. The source preserves the enabled check, duplicate scan, head insertion, size-derived allocation, copied fields, and forward link without instruction asm, barriers, register pins, non-mapped volatile, or opaque offset-heavy layout.
- The complete 92-byte linked `.text` section matched the target instruction-for-instruction. Normalized isolation reported only the legacy 10-byte target-symbol boundary; the narrow force path waived metadata only, and the integrated clean Docker ROM/report gate passed `wariowareinc.gba: OK` with ROM/base SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.
- Report progress advanced **1723 / 5922 → 1724 / 5921** matched functions, **77066 / 993856 → 77158 / 993856** matched code, **1263 / 5424 → 1264 / 5423** linked C/asm-only units, and **1464 → 1465** decomp files. The function denominator dropped by one when the converted legacy local-label boundary stopped being inferred as a separate function.
...
## Batch 302 — one exact ordinary-C linked-list removal helper (2026-08-25)
- Converted `func_08007EDC` from `asm/asm_08007edc.s` to a strict ordinary-C standalone TU using the named linked-list node shape shared by `func_08007F20` and `func_08007EAC`. An explicit early-null test plus do-while loop preserves predecessor tracking, head/non-head relinking, payload/node deallocation order, and the interworking epilogue without instruction asm, barriers, register pins, non-mapped volatile, or opaque layout.
- The first natural while-loop spelling was a genuine 64-byte near miss because it moved predecessor advancement into the loop preheader and changed the target branch layout. The accepted v2 spelling preserved the complete 68-byte section; isolation then differed only by the legacy 14-byte target-symbol boundary. The narrow force path waived metadata only, and the integrated clean Docker ROM/report gate passed with ROM/base SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.
- Report progress advanced **1722 / 5923 → 1723 / 5922** matched functions, **76998 / 993856 → 77066 / 993856** matched code, **1262 / 5425 → 1263 / 5424** linked C/asm-only units, and **1463 → 1464** decomp files. The function denominator dropped by one when the converted legacy local-label boundary stopped being inferred as a separate function.
...
- Report progress advanced **1720 / 5925 → 1721 / 5924** matched functions, **76914 / 993856 → 76962 / 993856** matched code, **1260 / 5427 → 1261 / 5426** linked C/asm-only units, and **1461 → 1462** decomp files. The function denominator dropped by one when the converted legacy local-label boundary stopped being inferred as a separate function.
- Evidence: `.decomp-runs/round-171-isolation.json`, `.decomp-runs/round-171-08007EAC-v1-source-audit.json`, `.decomp-runs/round-171-08007EAC-apply.json`, `.decomp-runs/round-171-full-source-audit.json`, and `.nearmiss/func_08007EAC.json`. Round170's rejected `func_08007E68` full-context mismatch remains documented in the tooling feedback log and was not accepted.
...
- Converted `func_08004C94` from `asm/asm_08004c94.s` to a strict ordinary-C standalone TU. A four-word local array provides the target's aligned `0x14` stack frame while its first three entries carry the incoming task words; a separate `u16 id` local preserves the early normalization and target store order without instruction asm, barriers, register pins, non-mapped volatile, or opaque layout.
- Added the canonical `D_083A49EC = 0x083A49EC` linker-map definition from `include/undefined_syms.inc`. The isolated linked-ELF screen's only mismatch was the legacy symbol-boundary metadata; resolving the symbol in Docker produced identical complete 40-byte linked `.text` sections with SHA-256 `8eb8d7ed27b4832c77715c2a11e49b34df148f72e87e641b0548a2c5d094fa72`, and the clean Docker ROM/report gate passed with equal ROM/base ROM SHA-1.
- Fresh report is **1720 / 5925** functions and **76914 / 993856** matched code. Unit coverage is **1260 C / 5427 asm-only**; decomp files are **1461** (**1241 standalone_tu / 220 included_stub**). Total units remain **6687**; the report denominator is one lower than Batch 298. ROM SHA-1 remains `3f556448d290fa5406d6ed367fee16cc02387ad3`.
...
- Fresh report is **1713 / 5927** functions and **76666 / 993844** matched code. Unit coverage is **1253 C / 5434 asm-only**; decomp files are **1454** (**1234 standalone_tu / 220 included_stub**). ROM SHA-1 remains `3f556448d290fa5406d6ed367fee16cc02387ad3`.
...
- Added the canonical `D_083A49E8 = 0x083A49E8` standalone linker-map definition, which was already present in `include/undefined_syms.inc`. Normalized isolation reported a target symbol of 14 bytes versus a candidate symbol of 20 bytes because the legacy target's local pool label ends the inferred function early. Docker-assembled target and linked candidate `.text` sections were independently extracted at the complete 20-byte size and matched byte-for-byte with SHA-256 `6db389b7381b9ff389dfe0368cb9226a19ca82d074650ce19cb86597f11551be`; the guarded metadata-only force path waived only that boundary. The clean Docker ROM/report gate passed with equal ROM/base ROM SHA-1.
- Fresh report is **1710 / 5928** functions and **76594 / 993844** matched code. Unit coverage is **1250 C / 5437 asm-only**; decomp files are **1451** (**1231 standalone_tu / 220 included_stub**). ROM SHA-1 remains `3f556448d290fa5406d6ed367fee16cc02387ad3`.
...
- Normalized linked-ELF isolation reported a target symbol of 26 bytes versus a candidate symbol of 38 bytes because the legacy target's internal return label ends the inferred function early. Docker-compiled target and candidate `.text` sections were independently extracted at the complete 40-byte size and matched byte-for-byte with SHA-256 `2df6ce90e997e4751ec7b273acbecec807b9ed0294e0f6c97cebbfd76fb68c62`; the guarded metadata-only force path waived only that boundary. The clean Docker ROM/report gate passed with equal ROM/base ROM SHA-1.
- Fresh report is **1708 / 5930** functions and **76546 / 993842** matched code. Unit coverage is **1248 C / 5439 asm-only**; decomp files are **1449** (**1229 standalone_tu / 220 included_stub**). ROM SHA-1 remains `3f556448d290fa5406d6ed367fee16cc02387ad3`.
...
- Fresh report is **1706 / 5932** functions and **76476 / 993840** matched code. Unit coverage is **1246 C / 5441 asm-only**; decomp files are **1447** (**1227 standalone_tu / 220 included_stub**). ROM SHA-1 remains `3f556448d290fa5406d6ed367fee16cc02387ad3`.
...
- Clean Docker build, report regeneration, `gen_objdiff.py`, and `decomp_cycle.py verify --no-report` passed with `wariowareinc.gba: OK`; ROM/base ROM SHA-1 remained `3f556448d290fa5406d6ed367fee16cc02387ad3`.
- Matching report metrics remain **1704 / 5934** functions and **76392 / 993840** code because all three were already C-linked. Pin residue drops **120 → 117 files / 571 → 560 pins**; full strict barrier residue remains **32 files / 36 findings**. Receipts: `.decomp-runs/round-148-isolation-v12.json`, `.decomp-runs/round-148-accepted-source-audit.json`, `.decomp-runs/round-148-full-source-audit.json`, `.decomp-runs/round-148-verify.json`, and `.decomp-runs/round-148-accepted-manifest.json`.
...
- Clean Docker build, report regeneration, `gen_objdiff.py`, and `decomp_cycle.py verify --no-report` passed with `wariowareinc.gba: OK`; ROM/base ROM SHA-1 remained `3f556448d290fa5406d6ed367fee16cc02387ad3`.
- Matching report metrics remain **1704 / 5934** functions and **76392 / 993840** code because all three were already C-linked. Pin residue drops **126 → 123 files / 595 → 584 pins**; full strict barrier residue drops **33 → 32 files / 37 → 36 findings**. Receipts: `.decomp-runs/round-146-isolation-v2.json`, `.decomp-runs/round-146-candidate-source-audit-all-v2.json`, `.decomp-runs/round-146-accepted-source-audit.json`, `.decomp-runs/round-146-full-source-audit.json`, `.decomp-runs/round-146-verify.json`, and `.decomp-runs/round-146-accepted-manifest.json`.
...
- The clean Docker ROM/report gate, local `gen_objdiff.py`, and `decomp_cycle.py verify --no-report` passed with `wariowareinc.gba: OK`; ROM/base ROM SHA-1 remained `3f556448d290fa5406d6ed367fee16cc02387ad3`.
- Matching report metrics remain **1704 / 5934** functions and **76392 / 993840** code because all three were already C-linked. Pin residue drops **145 → 142 files / 653 → 644 pins**; full strict barrier residue remains **35 files / 39 findings**. Receipts: `.decomp-runs/round-140-isolation.json`, `.decomp-runs/round-140-candidate-source-audit.json`, `.decomp-runs/round-140-accepted-source-audit.json`, `.decomp-runs/round-140-full-source-audit.json`, `.decomp-runs/round-140-verify.json`, and `.decomp-runs/round-140-accepted-manifest.json`.
...
- The clean Docker ROM/report gate, local `gen_objdiff.py`, and `decomp_cycle.py verify --no-report` passed with `wariowareinc.gba: OK`; ROM/base ROM SHA-1 remained `3f556448d290fa5406d6ed367fee16cc02387ad3`.
- Matching report metrics remain **1704 / 5934** functions and **76392 / 993840** code because both were already C-linked. Pin residue drops **150 → 148 files / 670 → 662 pins**; full strict barrier residue remains **35 files / 39 findings**. Receipts: `.decomp-runs/round-137-isolation.json`, `.decomp-runs/round-137-candidate-source-audit.json`, `.decomp-runs/round-137-accepted-source-audit.json`, `.decomp-runs/round-137-full-source-audit.json`, `.decomp-runs/round-137-verify.json`, and `.decomp-runs/round-137-accepted-manifest.json`.
...
- The clean Docker ROM/report gate, local `gen_objdiff.py`, and `decomp_cycle.py verify --no-report` passed with `wariowareinc.gba: OK`; ROM/base ROM SHA-1 remained `3f556448d290fa5406d6ed367fee16cc02387ad3`.
- Matching report metrics remain **1704 / 5934** functions and **76392 / 993840** code because all three were already C-linked. Pin residue drops **156 → 153 files / 693 → 681 pins**; full strict barrier residue remains **35 files / 39 findings**. Receipts: `.decomp-runs/round-135-isolation.json`, `.decomp-runs/round-135-candidate-source-audit.json`, `.decomp-runs/round-135-accepted-source-audit.json`, `.decomp-runs/round-135-full-source-audit.json`, `.decomp-runs/round-135-verify.json`, and `.decomp-runs/round-135-accepted-manifest.json`.
...
- The clean Docker ROM/report gate and `decomp_cycle.py verify --no-report` passed with `wariowareinc.gba: OK`; ROM/base ROM SHA-1 remained `3f556448d290fa5406d6ed367fee16cc02387ad3`.
- Matching report metrics remain **1704 / 5934** functions and **76392 / 993840** code because all three were already C-linked. Pin residue drops **168 → 165 files / 719 → 716 pins**; full strict barrier residue drops **36 → 35 files / 41 → 39 findings**. Receipts: `.decomp-runs/round-131-isolation.json`, `.decomp-runs/round-131-candidate-source-audit.json`, `.decomp-runs/round-131-accepted-source-audit.json`, `.decomp-runs/round-131-full-source-audit.json`, `.decomp-runs/round-131-verify.json`, and `.decomp-runs/round-131-accepted-manifest.json`.
...
- The clean Docker build/report gate and `decomp_cycle.py verify --no-report` passed with `wariowareinc.gba: OK`; ROM/base ROM SHA-1 remained `3f556448d290fa5406d6ed367fee16cc02387ad3`.
- Matching report metrics remain **1704 / 5934** functions and **76392 / 993840** code because all three were already C-linked. Pin residue drops **171 → 168 files / 725 → 719 pins**; full strict barrier residue remains **36 files / 41 findings**. Receipts: `.decomp-runs/round-130-isolation.json`, `.decomp-runs/round-130-accepted-source-audit.json`, `.decomp-runs/round-130-full-source-audit.json`, `.decomp-runs/round-130-verify.json`, and `.decomp-runs/round-130-accepted-manifest.json`.

## Batch 264 — three exact included-stub scene/table wrappers (2026-08-23)
- Rewrote `func_080024E4` with a named 12-byte table-entry overlay and ordinary cursor loop, plus `func_08016E6C` and `func_08016D88` with ordinary scene predicates, saved results, and typed globals. Six compiler register pins were removed without barriers, volatile accesses, or instruction asm.
- The eight-entry screen found **3 exact / 5 near miss** spellings; the selected three-entry subset passed the strict source audit with zero instruction asm, barriers, pins, non-mapped volatile accesses, or opaque layouts. Because these host stubs had already moved their original assembly into `asm/converted/`, the accepted change was a source-only included-stub refresh with no linker or host-order change.
- The clean Docker build/report gate and `decomp_cycle.py verify --no-report` passed with `wariowareinc.gba: OK`; ROM/base ROM SHA-1 remained `3f556448d290fa5406d6ed367fee16cc02387ad3`.
- Matching report metrics remain **1704 / 5934** functions and **76392 / 993840** code because all three were already C-linked. Pin residue drops **174 → 171 files** and **731 → 725 pins**; full strict barrier residue remains **36 files / 41 findings**. Receipts: `.decomp-runs/round-129-isolation.json`, `.decomp-runs/round-129-source-audit.json`, `.decomp-runs/round-129-accepted-source-audit.json`, `.decomp-runs/round-129-full-source-audit.json`, and `.decomp-runs/round-129-verify.json`.
...
- Rewrote `func_080F28F8` with ordinary `result`/`value` locals that preserve the target stack epilogue and arithmetic register homes, removing its R0 pin. Rewrote `func_0800557C` with a named four-word heap-record overlay and an explicit allocated-pointer return, removing its R0 pin.
...
- Rewrote `func_08003998` with an ordinary cursor local and sequential post-increment byte stores, `func_0801E44C` with ordinary data-base and sprite-handler locals, and `func_080F1FB4` with ordinary arithmetic temporaries. Six compiler register pins were removed while the target bytes stayed exact.
- The thirteen-entry screen found six exact candidates, one policy-clean near miss, and six strict-policy rejections. The final three-entry isolation screen was **3 exact / 0 rejected** under the strict source audit.
...
- The clean Docker ROM/report gate and `decomp_cycle.py verify` passed with `wariowareinc.gba: OK`; ROM/base ROM SHA-1 remained `3f556448d290fa5406d6ed367fee16cc02387ad3`.
- Matching report metrics remain **1704 / 5934** functions and **76392 / 993840** code because all three were already C-linked. Source residue is now **177 pin-bearing files / 737 pins** and **11 non-volatile empty-barrier files / 11 barriers**; the full strict audit reports **38 files / 45 empty barrier findings**. Receipts: `.decomp-runs/round-125-mask-screen.json`, `.decomp-runs/round-126-isolation.json`, `.decomp-runs/round-126-accepted-isolation.json`, and `.decomp-runs/round-126-verify.json`.
...
- The eight-entry candidate screen produced exact ordinary-C spellings for the selected three functions; the final three-entry exact-only screen was **3 exact / 0 rejected** under the strict source audit. The accepted sources contain named layout evidence or bounded raw evidence only.
- The clean Docker ROM/report gate and `decomp_cycle.py verify` passed with `wariowareinc.gba: OK`; ROM/base ROM SHA-1 remained `3f556448d290fa5406d6ed367fee16cc02387ad3`.
- Matching report metrics remain **1704 / 5934** functions and **76392 / 993840** code because all three were already C-linked. Source residue is now **174 pin-bearing files / 731 pins** and **11 non-volatile empty-barrier files / 11 barriers**; the full strict audit reports **36 files / 41 empty barrier findings**. Receipts: `.decomp-runs/round-127-isolation.json`, `.decomp-runs/round-128-isolation.json`, `.decomp-runs/round-128-accepted-isolation.json`, and `.decomp-runs/round-128-verify.json`.
...
## Batch 250 — strict included-stub main-menu task-spawn helper (2026-08-21)
- Converted `func_08012D7C` to guarded ordinary C: `scene_set_current_thread(0)`, five-word stack block (`VRAMBase+0x8000` ×2, `0x4000`, `0x1000`, `4`) passed to `start_new_task`, then `run_func_after_task` with the `func_08012D3C+1` thumb callback. Only named/typed symbols; no raw numeric scene offsets.
- First apply rolled back on a conflicting-types error: the main_menu host TU already declares `get_current_mem_id` as u16 via sibling decomp includes. Re-declared u16 (codegen-neutral after the existing u16 cast) and re-applied.
- Section audit: 48 instruction bytes exact; final pool word is link-time-relocation-identical. Full Docker gate: `wariowareinc.gba: OK`, ROM SHA-1 unchanged. Decomp files **1437 → 1438** (**1225 standalone_tu / 213 included_stub**); linked metrics unchanged.
...
- Corrected stale `extern void func_08011864(u8)` prototypes in `asm_080113bc.c` / `asm_080118e0.c` to `(u32)` — the target's non-widening prologue and unsigned `blo` switch dispatch prove a 32-bit parameter; call sites pass zero-extended bytes so their codegen is unchanged. Full Docker gate: `wariowareinc.gba: OK`, ROM SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3` unchanged. Decomp files **1435 → 1436** (**1225 standalone_tu / 211 included_stub**); linked metrics unchanged.
...
## Batch 238 — strict included-stub bitmap helper (2026-08-07)
- `func_0800C218` replaced the bitmap-font host TU's asm include with guarded ordinary C. The accepted source returns the existing helper's `void *` result, uses a unique all-`s16` typed function-pointer ABI and named `u32` truncation locals, and contains no wrapped asm, instruction asm, register pin, barrier, non-mapped volatile, or opaque offset-heavy pointer manipulation.
- Round 89's strict host-TU screen found two exact c218 spellings; v7 was selected over v8 because it preserves the semantic return type. The full apply receipt reports `wariowareinc.gba: OK`, `rom_exact: true`, and unchanged SHA-1 **`3f556448d290fa5406d6ed367fee16cc02387ad3`**.
- Included-stub report metrics remain **1691 / 5934** matched functions, **75984 / 993820** matched code, and **1231 C / 5456 asm-only** linked units. Decompiled files advance **1416 → 1417** (**204 → 205 included_stub**).
- `func_0800C3AC` (best isolated gap **13.046875**) and `func_0800DE84` (best gap **0.34884**, instruction bytes equal but literal-pool metadata differs) remain near-miss evidence only. Receipts include `round-89-source-audit-v1` through `v5`, `round-89-isolation-v1` through `v5`, the name-select evidence, and `round-89-c218-apply.json`.

## Batch 239 — strict included-stub main-menu wrapper (2026-08-07)
- Converted `func_080147B0` from `src/scenes/main_menu.c`'s asm include to guarded ordinary C. The selected source directly uses the existing typed `gMainMenu.unk13C_2` and `gMainMenu.unk1C4` fields plus typed helper declarations; it contains no wrapped asm, instruction asm, register pin, barrier, non-mapped volatile, or opaque offset-heavy pointer manipulation.
- Round 90 screened seven readable spellings. The selected direct-field form reached a **99.85%** isolated score, with identical instructions and only candidate-origin relocation metadata differing. The first guarded full-context attempt rolled back cleanly after exposing a conflicting local `s32 func_08011698()` declaration; changing it to the host TU's existing `u32` declaration allowed the complete Docker gate to pass `wariowareinc.gba: OK`, `rom_exact: true`, and ROM SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.
- The strict audit reports zero instruction asm, barriers, compiler register pins, non-mapped volatile accesses, raw pointer accesses, and numeric pointer-offset lines. Report metrics remain **1691 / 5934** and **75984 / 993820** because the included stub was already matching in its host TU; decomp-file coverage advances **1417 → 1418** (**205 → 206 included_stub**). The six rejected readable variants, isolation receipts, rollback receipt, accepted receipt, near-miss source, and attempt-ledger row are retained as provenance.

## Batch 240 — strict standalone wrapper/leaf batch (2026-08-07)
- Converted `func_0800C704`, `func_0800C720`, `func_080D6D28`, and `func_08064D10` to standalone ordinary C. The C704/C720 pair uses typed sentinel-table cursors, D6D28 preserves the target signed threshold branch, and D64D10 models its two accessed record fields with a named overlay. No wrapped asm, instruction asm, barrier, register pin, non-mapped volatile, or opaque offset-heavy pointer manipulation was used.
- Round 91 screened **13** candidate spellings across two isolation passes and classified **4 exact / 9 near miss**. m2c supplied the semantic skeletons. asmlift declined the `LDM R4!` walkers as an unsupported effect and failed at project-context compilation for the other diagnostic candidates; no generated asmlift source was admitted. The m2c `s32 * += 4` walker advance was corrected to `cursor++` by checking the target's post-increment load semantics.
- The strict audit reports zero instruction asm, barriers, compiler register pins, non-mapped volatile accesses, and numeric pointer-offset lines in all four accepted files. Each walker has one typed pointer cast and no numeric offset; D64D10 uses a named overlay. The clean full Docker gate passed `wariowareinc.gba: OK`, `rom_exact: true`, and ROM SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.
- Report progress is **1691 → 1695** matched functions, **75984 → 76092** matched code, and **1231 → 1235** linked C TUs. Decompiled files advance **1418 → 1422** (**1212 → 1216 standalone_tu**, included stubs remain **206**). The three bitfield/mask candidates remain near-miss evidence with their receipts and attempt-ledger rows.

## Batch 241 — strict ordinary-C wrapper/table batch (2026-08-07)
- Converted `func_080194D8`, `func_08022070`, and `func_080DF224` to standalone ordinary C. The call/key wrapper preserves its four-call order and key-bit extraction, the table helper uses a named root plus padded 0x20-byte entry overlay, and the sound wrapper keeps the target's `u16` third-argument ABI and store order. No wrapped asm, instruction asm, barrier, register pin, non-mapped volatile, or opaque offset-heavy pointer manipulation was used.
- Round 92 screened **9** candidate spellings in one isolation pass and classified **3 exact / 6 near miss**. m2c supplied the semantic skeletons. asmlift was diagnostic only: project-context compile failures affected two wrappers, stack address-taking caused a supported-effect decline for the forwarding wrapper, and its table/fixed-point outputs were not exact. No generated asmlift source was admitted.
- The strict accepted-source audit reports zero instruction asm, barriers, compiler register pins, non-mapped volatile accesses, and opaque offset-heavy layout in all three files. The table's 0x20 stride is represented by a named padded record and `entry++`, not raw byte-pointer arithmetic. The full Docker gate passed `wariowareinc.gba: OK`, `rom_exact: true`, and ROM SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.
- Report progress is **1695 → 1698** matched functions, **76092 → 76192** matched code, and **1235 → 1238** linked C TUs. Decompiled files advance **1422 → 1425** (**1216 → 1219 standalone_tu**, included stubs remain **206**). The stack-forwarder and fixed-point forms remain evidence-only in `.nearmiss/` and the Round 92 isolation receipt.

## Batch 242 — strict named-record leaf batch (2026-08-07)
- Converted `func_08089148`, `func_080B39F0`, `func_080CF440`, `func_080CF6C0`, and `func_0801CB24` to standalone ordinary C. The accepted forms use a named 0x40-byte entry stride, named packed-record fields, explicit locals to preserve the fixed-point load order, and the target ABI's widened `get_random_range` declaration. No wrapped asm, instruction asm, barrier, register pin, non-mapped volatile, or raw/numeric pointer layout was used.
- Round 93 screened **13** spellings across two isolation passes and classified **5 exact / 8 near miss**. m2c supplied the semantic skeletons. asmlift remained diagnostic-only because its project-context compile/scoring boundary rejected the generated candidates; no asmlift source was admitted. The second pass selected the ordinary-C mask/load-order/ABI spellings that reproduced the bytes.
...
- Report progress is **1698 → 1703** matched functions, **76192 → 76334** matched code, and **1238 → 1243** linked C TUs. Decompiled files advance **1425 → 1430** (**1219 → 1224 standalone_tu**, included stubs remain **206**). The allocation/scene-store, mask, and narrow-ABI alternatives remain evidence-only near misses with their Round 93 receipt/ledger rows.

## Batch 243 — strict allocator-record leaf (2026-08-07)
- Converted `func_0800C73C` to standalone ordinary C. The function allocates a named five-halfword record, stores the helper result and defaults in target order, and returns the typed record pointer. No wrapped asm, instruction asm, barrier, register pin, non-mapped volatile, or opaque numeric-offset layout was used.
- Round 94 screened **13** candidates in one isolation pass and classified **1 exact / 12 near miss**. The sentinel pointer-list spelling differs in its `-1` literal-pool/constant shape; the three 0x20-byte flag-table pairs remain register/mask-layout near misses even with named entry overlays. No rejected spelling was forced into the source tree.
- The strict accepted-source audit reports zero instruction asm, barriers, compiler register pins, non-mapped volatile accesses, raw pointer accesses, and numeric pointer offsets. The full Docker gate passed `wariowareinc.gba: OK`, `rom_exact: true`, and ROM SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.
- Report progress is **1703 → 1704** matched functions, **76334 → 76372** matched code, and **1243 → 1244** linked C TUs. Decompiled files advance **1430 → 1431** (**1224 → 1225 standalone_tu**, included stubs remain **206**). Evidence is retained in the Round 94 isolation/apply/source-audit receipts, `.nearmiss/`, and `tools/attempts.tsv`.
...
## Batch 253 — name-select sprite-create wrapper (2026-08-21)
- Converted `func_0800DE84` (name_select scene) from an included-stub asm file to ordinary C: `sprite_create(gSpriteHandler, func_080049BC(scene->unk8, arg0), 0, (s16)arg1, (s16)arg2, 0x800, 0, 0, 0)` followed by `sprite_set_visible(gSpriteHandler, id, 0)`, returning the s16 id.
- The round-89 near-miss spelling was re-audited and found byte-identical on a raw standalone `.text` section comparison. The isolate harness still scored it 0.34884 because its `--unresolved-symbols=ignore-all` candidate link resolves the two `BL` callees (`func_080049BC`, `sprite_set_visible`) to different displacements than the target TU link. Equivalence was proven by linking both the extracted target `.s` object and the candidate object with identical `--defsym` callee addresses: the 92-byte function bytes compared equal.
- Admitted through the documented metadata-only `--force` boundary path (batch 220 precedent). The transactional clean Docker rebuild emitted `wariowareinc.gba: OK` with `rom_exact: true`, preserving ROM SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.
- Fresh metrics are **1704 / 5934** matched functions (**28.715876%**), **76382 / 993830** matched code (**7.6856203%**), **1244 C / 5443 asm-only** units, and **1441** decomp files (`1225 standalone_tu` + `216 included_stub`). Evidence: `.decomp-runs/20260821T234936Z-isolation.json` and `.decomp-runs/20260821T235544Z-apply-func_0800DE84.json`.

## Batch 254 — main-menu scene-tick helper func_08011DFC (2026-08-22)
- Converted `func_08011DFC` (main_menu) from an included-stub asm file to ordinary C: a scene-tick loop that calls `(*pp)->unkF4((*pp)->unkF9)` once per iteration, pre-increments the counter byte, and clears the enable flag when the counter reaches its limit (`1` iteration normally, `3` when the limit exceeds `0x1B`).
- Shaping that reached the exact 120-byte body: (1) an initial `scene = SCENE;` read for both early checks, dead after `count`; (2) a do-while with a pre-test (`i = 0; if (i < count) { pp = ...; zero = 0; do { ... } while (i < count); }`) so the anchor copy (`ADDS R5,R1,#0`) and `MOVS R7,#0` are emitted once, in that order, between the entry test and the loop body; (3) a typed address local `pp = (struct Func11DfcData **)&gCurrentSceneData;` whose pool load CSE-reuses r1 from the initial section; (4) direct `(*pp)->...` expressions inside the body so every access reloads `[r5]` fresh (nothing pointer-live across the indirect call); (5) inline `++(*pp)->unkF9 >= (*pp)->unkFA` so the limit address computes before the zero-extension interleave.
- Isolation scored 0.09615 solely on the harness's `_call_via_r1` displacement; a raw standalone section audit plus a `--defsym=_call_via_r1=` linked comparison showed zero differing bytes. Admitted through the documented metadata-only `--force` boundary path; the clean Docker rebuild emitted `wariowareinc.gba: OK` with `rom_exact: true`.
- Metrics: **1704 / 5934** matched functions (**28.715876%**), **76382 / 993830** matched code (**7.6856203%**), **1244 C / 5443 asm-only** units, **1442** decomp files (**1225 standalone_tu + 217 included_stub**). Evidence: `.decomp-runs/20260822T001531Z-isolation.json` (5-spelling screen), `20260822T002017Z-isolation.json`, and `20260822T002154Z-apply-func_08011DFC.json`.
...
- All three candidates passed the strict real-C and layout audits with zero instruction asm, barriers, register pins, or non-mapped volatile. A first transaction rolled back on a shared-host declaration conflict (`D_083A4A1C` scalar versus the existing array declaration); matching the host declaration and rerunning the transaction fixed the issue without changing generated bytes.
- The linked isolation screen reported only the known candidate-only BL displacement artifact: `0.28571`, `0.454544`, and `0.408165` gaps for the three wrappers. Raw function text and literal-pool bytes matched the target host object, and the full Docker gate accepted the batch with `wariowareinc.gba: OK`, `rom_exact: true`, and ROM SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.
