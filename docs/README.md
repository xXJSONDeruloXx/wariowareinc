...
- Verified working tree: `batch 261` — three additional ordinary-C wrappers (`func_08003998`, `func_0801E44C`, and `func_080F1FB4`) now emit the same bytes without compiler register pins
...
- Latest accepted maintenance pass: **35 legacy included-stub files** use real C and ABI/register shaping instead of non-empty inline-asm call/load shims; batches 256–261 additionally removed twenty-nine compiler register pins across seventeen wrappers. Report function/unit metrics are unchanged because these files were already C-linked.
- Remaining naked/original asm wrapper files in `src/decomp`: **0**; remaining compiler-register-pin files: **180 files / 743 pins**
