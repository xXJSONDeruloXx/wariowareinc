...
- Verified working tree: `batch 263` — three additional ordinary-C nested-scene/graphics/runtime wrappers (`func_0801D4A0`, `func_0805CB5C`, and `func_080F0DE0`) now emit the same bytes without compiler register pins or empty barriers
...
- Latest accepted maintenance pass: **35 legacy included-stub files** use real C and ABI/register shaping instead of non-empty inline-asm call/load shims; batches 256–263 additionally removed forty-one compiler register pins across twenty-three wrappers. Report function/unit metrics are unchanged because these files were already C-linked.
- Remaining naked/original asm wrapper files in `src/decomp`: **0**; remaining compiler-register-pin files: **174 files / 731 pins**
- Remaining non-volatile empty compiler barriers: **11 files / 11 barriers**; the full strict audit also reports **36 files / 41 empty barrier findings** when volatile barriers coexisting with legacy pins are included. Remaining instruction-bearing inline-asm decomp files: **0**
