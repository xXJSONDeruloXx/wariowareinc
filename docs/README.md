...
2. **`make report` says ~5.41% matched code**, which is close to the user estimate, but that is **not the same thing as decompiled C coverage**.
3. A rough, repo-local heuristic based on current C definitions puts **explicit C function coverage closer to ~2.75% by function count** (`164 / 5961`).
...
6. Two real asm→C smoke tests were completed and kept the ROM matching:
   - `src/memory_heap.c` → `mem_heap_alloc`
   - `src/beatscript.c` → `func_0800A270`
7. A repo-local Mizuchi bootstrap was added:
   - `mizuchi.yaml`
