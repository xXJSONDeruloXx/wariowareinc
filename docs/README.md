...
- Verified working tree: `batch 264` — three additional included-stub scene/table wrappers (`func_080024E4`, `func_08016E6C`, and `func_08016D88`) now emit the same bytes without compiler register pins
...
- Latest accepted maintenance pass: **35 legacy included-stub files** use real C and ABI/register shaping instead of non-empty inline-asm call/load shims; batches 256–264 additionally removed forty-seven compiler register pins across twenty-six wrappers. Report function/unit metrics are unchanged because these files were already C-linked.
- Remaining naked/original asm wrapper files in `src/decomp`: **0**; remaining compiler-register-pin files: **171 files / 725 pins**
