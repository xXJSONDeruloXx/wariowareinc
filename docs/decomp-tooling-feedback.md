...
- `func_0800CAA4` and `func_0800CAB8` use separate ordinary base/offset locals; `func_0800D224` uses base/offset/field locals. All three are strict-policy clean and remove six pins plus four empty barrier expressions across three barrier-bearing files without changing the instruction streams.
- These were already C-linked units, so the report remains **1704 / 5934** functions, **76392 / 993840** matched code, and **1244 C / 5443 asm-only** units. Source residue drops **180 → 177** pin-bearing files, **743 → 737** pins, and the tracked barrier-file cleanup count drops **11 → 8**.
