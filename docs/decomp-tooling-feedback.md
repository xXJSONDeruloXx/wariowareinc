...
- `.githooks/pre-commit` and `.githooks/pre-push` call the same verifier for ROM-affecting changes, while tooling/docs-only commits skip Docker. `tools/check_decomp_policy.py` independently blocks new non-empty inline asm and verification-gate edits. This complements, rather than replaces, `.pi/extensions/warioware-decomp-guard.js`.
