...
- The durable compiler-shaping lesson is to preserve handler-before-scene load order, use compiler-only hard-register declarations for the target's scene pointer and offset/base roles, and keep the countdown as a widened `u32` so the target's store precedes its `LSLS #16` zero test. These declarations emit no instructions and no volatile asm.
