...
## Round 186 — DMA wrapper and metadata-only literal-pool boundary (2026-08-25)
- The historical `func_08006F04` candidate was re-screened with the current strict policy. Its ordinary-C `dma3_set` body had no executable mismatch; normalized isolation's 14.28571 score came solely from the legacy target symbol ending at 30 bytes while the complete candidate section included the 0x0000 alignment halfword and `D_03004054` literal word through 36 bytes.
- `D_03004054` was already available through the canonical project header/linker mapping, so no new symbol workaround was needed. A Docker full-text proof linked the target with thumb-typed `dma3_set` at `0x0800100c` and matched the full 36-byte section at SHA-256 `978e31f6b5e6c95d0d1e7567c85667a7c2cafe23eef05e9a25c06a10296764cf`; the force path waived symbol metadata only.
...
- Evidence: `.decomp-runs/round-186-isolation.json`, `.decomp-runs/round-186-func_08006F04-v1-source-audit.json`, `.decomp-runs/round-186-func_08006F04-apply.json`, `.decomp-runs/round-186-func_08006F04-full-text-proof.json`, `.decomp-runs/round-186-full-source-audit.json`, and `.nearmiss/func_08006F04.json`.
...
- Normalized isolation reported only the known legacy target-symbol boundary, 26 target bytes versus a complete 32-byte literal-pool section. The raw target object retains an unresolved `R_ARM_THM_CALL` relocation, so a direct object-byte comparison would falsely differ at the `BL`. The complete Docker proof linked a thumb-typed `start_new_task` symbol at its actual `0x08005790` address before comparing the whole section; target and ROM bytes matched at SHA-256 `2a7c2d01ec0f14356fb13ea593053856d94b05b5c3987fe20ab9f52b0c05a423`.
- The clean Docker gate passed with equal ROM/base SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`. Full strict audit remains at zero instruction asm, **106 files / 502 register pins**, **27 files / 31 barrier findings**, and **7 non-volatile barriers**; the accepted candidate audit is strict-clean.
- Evidence: `.decomp-runs/round-184-isolation.json`, `.decomp-runs/round-184-start_new_texture_loader-v1-source-audit.json`, `.decomp-runs/round-184-start_new_texture_loader-apply.json`, `.decomp-runs/round-184-start_new_texture_loader-full-text-proof.json`, `.decomp-runs/round-184-full-source-audit.json`, and `.nearmiss/start_new_texture_loader.json`.

## Round 183 — byte-mask liveness and literal-pool boundary (2026-08-25)
- The first natural candidates for `func_08003FB8` exposed two separate ordinary-C code-generation traps. A `u32` loaded value preserved the target's `MOVS #9; RSBS` pair but changed register homes; a `u8` value gave the right homes but folded the second negation to `MOVS #0xF7`. The accepted spelling keeps the loaded byte in `u8 value`, the first result in `u32 mask`, and the second negation in a separate `u32 other`; normal liveness reuses the value register for `other` and emits the target's ten instructions without pins, barriers, volatile, or asm.
- The linker symbol `D_03000528` was present in `include/undefined_syms.inc` but absent from `undefined_syms.ld`; adding the canonical absolute assignment was required for the standalone C TU's literal relocation.
- Normalized linked-ELF isolation reported only the known legacy target-symbol boundary: 20 target bytes versus the complete 24-byte candidate section. A Docker full-text proof matched both complete sections at SHA-256 `0088ecc7f086adfa14e43fef9d2490c0909a14a76ec0958607372ac7d784b539`; the full ROM gate then passed with equal ROM/base SHA-1. The force path waived symbol metadata only.
- Full strict audit remains at zero instruction asm, **106 files / 502 register pins**, **27 files / 31 barrier findings**, and **7 non-volatile barriers**. The accepted source audit is strict-clean.
...
## Round 182 — sound-player table scan and converted-callee integration (2026-08-25)
- `func_08002090` reached exact instruction bytes with a named 0xC-byte sound-player table entry, separate fixed-point `value`, and a count pointer assigned only after the early `sound_player_count` check. That source order kept the initial count address in R1/R2 and copied it to R7 after the table literal, matching the target without a register pin or barrier.
- Normalized linked-ELF isolation reports a near miss only because the legacy target symbol ends at internal label `_080020A6` after 22 bytes while the complete candidate section is 56 bytes. The full Docker transaction was authoritative and passed with equal ROM/base SHA-1 even though `func_080F30E0` is already a converted C callee; no isolated-only acceptance was used.
- Full strict audit remains at zero instruction asm, **106 files / 502 register pins**, and **27 files / 31 barrier findings** including **7 non-volatile barriers**.
...
## Round 181 — decimal formatter and legacy local-label boundary (2026-08-25)
- `func_08007B98` required a genuine source-shape correction: the first C spelling reused the digit-count quotient as the reverse-pass value, while the accepted spelling keeps a separate `number` local. Introducing a local `pair = D_083A4B10 + digit * 2` after `__umodsi3` also preserved the target's repeated table-literal load without adding a saved high register; the resulting 92-byte section is ordinary C and strict-audit clean.
- Normalized linked-ELF isolation still reports a near miss because the legacy target object's `func_08007B98` symbol is inferred to stop at internal label `_08007BA6` after 14 bytes, while the candidate symbol spans the complete 92-byte section. The recorded diff shows the complete target/candidate section sizes, and the clean Docker transaction accepted the conversion with equal ROM/base SHA-1. This is the same metadata-only class as earlier local-label cases; it is not an instruction mismatch and does not weaken the no-cheap-tricks policy.
- The table address was already canonical in `include/undefined_syms.inc` but missing from `undefined_syms.ld`; adding `D_083A4B10 = 0x083A4B10` was required for the real standalone link. Full strict audit remains at zero instruction asm, **106 files / 502 register pins**, and **27 files / 31 barrier findings** including **7 non-volatile barriers**.
- Evidence: `.decomp-runs/round-181-isolation.json`, `.decomp-runs/round-181-isolation-v2.json`, `.decomp-runs/round-181-isolation-v3.json`, `.decomp-runs/round-181-isolation-v5.json`, `.decomp-runs/round-181-func_08007B98-v5-source-audit.json`, `.decomp-runs/round-181-func_08007B98-apply.json`, `.decomp-runs/round-181-full-source-audit.json`, `.nearmiss/func_08007B98.json`, and `.nearmiss/func_08007B98.full.c`.

## Round 177 — scheduled task wrapper, named stack record, and canonical literal (2026-08-25)
- The first `schedule_function_call` candidate was correctly rejected by the strict layout gate because a raw four-word array plus indexed address exposed six opaque numeric accesses. A named three-field task-argument record passed the strict source audit and preserved the target's `SUB SP,#0x10` frame, three stores, outgoing zero argument, and `start_new_task` call.
- A non-void `return start_new_task(...)` spelling was required to preserve the target `POP {R1}; BX R1` epilogue. The complete 40-byte linked `.text` section then matched instruction-for-instruction; normalized isolation reported only the legacy 34-byte target-symbol boundary. The canonical `D_083A4B38` linker assignment was added, and the integrated clean Docker transaction passed `wariowareinc.gba: OK` with equal ROM/base SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.
- The accepted conversion advanced the report from **1725 / 5920** to **1726 / 5919**, matched code from **77194 / 993856** to **77234 / 993858**, linked C units from **1265 / 5422** to **1266 / 5421**, and decomp files from **1466** to **1467**. The denominator drops by one while total code rises by two because the converted legacy literal-pool boundary is now counted in the report's code/function units.
- Evidence: `.decomp-runs/round-177-isolation.json`, `.decomp-runs/round-177-isolation-v2.json`, `.decomp-runs/round-177-isolation-v3.json`, `.decomp-runs/round-177-isolation-v4.json`, `.decomp-runs/round-177-schedule_function_call-v4-source-audit.json`, `.decomp-runs/round-177-schedule_function_call-apply.json`, `.decomp-runs/round-177-full-source-audit.json`, `.nearmiss/schedule_function_call.json`, and `.nearmiss/schedule_function_call.full.c`.

## Round 176 — scheduled-task updater and indirect-call integration (2026-08-25)
- `update_scheduled_function_task` was a strict-clean ordinary-C updater using a named three-field task record. The candidate preserved the target's delay decrement branch, nullable callback check, `_call_via_r2` shape, and complete 36-byte linked `.text` section; normalized objdiff reported only the legacy 18-byte target-symbol boundary. No instruction asm, barrier, register pin, non-mapped volatile, or opaque layout was introduced.
- The guarded force path waived only that target-symbol boundary. Unlike the earlier `func_08007E68` negative screen, the integrated clean Docker transaction passed `wariowareinc.gba: OK`, generated the report, and preserved equal ROM/base SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.
- The accepted conversion advanced the report from **1724 / 5921** to **1725 / 5920**, matched code from **77158 / 993856** to **77194 / 993856**, linked C units from **1264 / 5423** to **1265 / 5422**, and decomp files from **1465** to **1466**. The total-function denominator fell by one because the converted legacy local-label boundary is no longer inferred as a separate function.
- Evidence: `.decomp-runs/round-176-isolation.json`, `.decomp-runs/round-176-update_scheduled_function_task-v1-source-audit.json`, `.decomp-runs/round-176-update_scheduled_function_task-apply.json`, `.decomp-runs/round-176-full-source-audit.json`, `.nearmiss/update_scheduled_function_task.json`, and `.nearmiss/update_scheduled_function_task.full.c`.

## Round 175 — string-append register-order screen (2026-08-25)
- The adjacent `func_08007AF8` string-append helper remains evidence-only. Several ordinary-C two-pointer spellings reproduced the complete 44-byte control flow and tail but retained a genuine two-instruction prologue-order mismatch (`R3 <- R0; R2 <- R3` versus target `R2 <- R0; R3 <- R2`). No register pin, barrier, volatile, or asm workaround was used; `.decomp-runs/round-175-isolation.json` and its v3-v7 receipts plus the pre-existing `.nearmiss/func_08007AF8.json` preserve the negative evidence.

## Round 174 — linked-list cache-entry creator and symbol-boundary metadata (2026-08-25)
- `func_08007F48` was a strict-clean ordinary-C cache-entry creator using named input/resource/entry overlays. The isolated candidate's complete 92-byte linked `.text` section matched the target instruction-for-instruction, while normalized objdiff still reported a near miss because the legacy target symbol inferred only 10 bytes. No instruction asm, barrier, register pin, non-mapped volatile, or opaque layout was introduced.
- The guarded force path waived only that target-symbol boundary. The integrated clean Docker transaction passed `wariowareinc.gba: OK`, generated the report, and preserved equal ROM/base SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.
- The accepted conversion advanced the report from **1723 / 5922** to **1724 / 5921**, matched code from **77066 / 993856** to **77158 / 993856**, linked C units from **1263 / 5424** to **1264 / 5423**, and decomp files from **1464** to **1465**. The total-function denominator fell by one because the converted legacy local-label boundary is no longer inferred as a separate function.
- Evidence: `.decomp-runs/round-174-isolation.json`, `.decomp-runs/round-174-08007F48-v1-source-audit.json`, `.decomp-runs/round-174-08007F48-apply.json`, `.decomp-runs/round-174-full-source-audit.json`, `.nearmiss/func_08007F48.json`, and `.nearmiss/func_08007F48.full.c`.

## Round 173 — linked-list removal loop shaping (2026-08-25)
- `func_08007EDC` reused the named node overlay from the adjacent list helpers. The first ordinary-C while-loop spelling was strict-clean but a genuine near miss: agbcc placed the predecessor update before the loop's initial null check, producing a 64-byte body instead of the target's 68-byte control-flow layout. No pin, barrier, volatile, or asm workaround was used.
- An explicit `if (node != 0)` around a do-while scan preserved the target's initial `CMP/BEQ`, `R1` predecessor lifetime, head/non-head relinking, two deallocations, and complete 68-byte section. Normalized isolation reported only the legacy 14-byte target-symbol boundary; the guarded force path waived metadata only, and the full Docker ROM/report gate passed `wariowareinc.gba: OK` with equal ROM/base SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.
- The accepted conversion advanced the report from **1722 / 5923** to **1723 / 5922**, matched code from **76998 / 993856** to **77066 / 993856**, linked C units from **1262 / 5425** to **1263 / 5424**, and decomp files from **1463** to **1464**. The total-function denominator fell by one because the converted legacy local-label boundary is no longer inferred as a separate function.
- Evidence: `.decomp-runs/round-173-isolation.json`, `.decomp-runs/round-173-isolation-v2.json`, `.decomp-runs/round-173-08007EDC-v1-source-audit.json`, `.decomp-runs/round-173-08007EDC-v2-source-audit.json`, `.decomp-runs/round-173-08007EDC-apply.json`, `.decomp-runs/round-173-full-source-audit.json`, and `.nearmiss/func_08007EDC.json`.

## Round 172 — bounded byte-copy wrapper and historical prologue trap (2026-08-25)
- The old Round55 `func_08007AD4` candidate was correctly rejected because its two saved-register copies were reversed. A fresh strict-clean v1 spelling keeps the original destination in `result`, the unsigned bound in `limit`, and the loaded byte in `value`; agbcc then emits the target `R4`/`R5` order, single-byte load/store, pointer increments, and `u32` counter loop with no asm, barriers, pins, non-mapped volatile, or opaque layout.
- Normalized linked-ELF isolation reported only the legacy 10-byte function-symbol boundary versus the complete 36-byte candidate section. The guarded force path waived metadata only; the full Docker ROM/report gate passed `wariowareinc.gba: OK` with equal ROM/base SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.
- The accepted conversion advanced the report from **1721 / 5924** to **1722 / 5923**, matched code from **76962 / 993856** to **76998 / 993856**, linked C units from **1261 / 5426** to **1262 / 5425**, and decomp files from **1462** to **1463**. The total-function denominator fell by one because the converted legacy local-label boundary is no longer inferred as a separate function.
- Evidence: `.decomp-runs/round-172-isolation.json`, `.decomp-runs/round-172-08007AD4-v1-source-audit.json`, `.decomp-runs/round-172-08007AD4-apply.json`, `.decomp-runs/round-172-full-source-audit.json`, and `.nearmiss/func_08007AD4.json`. The historical Round55 near-miss receipt remains in the existing `.nearmiss` record.

## Round 171 — linked-list teardown and full-context function-pointer trap (2026-08-25)
- `func_08007E68` was a useful negative screen. The strict-clean named-state candidate preserved the target's indirect `gfx_decompress_rom` call shape and received an isolated score of 100, but the guarded full Docker transaction produced candidate ROM SHA-1 `d48878ffb52a3fabc0d0ef5839662a248f89395d` instead of the baseline `3f556448d290fa5406d6ed367fee16cc02387ad3`, then rolled back all source/linker edits. This is an integration mismatch, not a metadata-only exception; retain the candidate and receipt as evidence and do not force it. A disposable worktree rerun also stopped before compilation because a manually assembled Docker command did not provide the project's DevKit header environment, so it adds no binary evidence.
- `func_08007EAC` reused the named linked-list node model from `func_08007F20`: payload at `+4`, successor at `+0x14`, successor saved before the two frees, and global cleared after the loop. The strict audit passed with zero instruction asm, barriers, pins, non-mapped volatile, or opaque layout. Isolation reported only the target's truncated 10-byte symbol boundary versus the complete 48-byte section; the guarded metadata-only force path still ran the full Docker ROM/report gate and accepted equal ROM SHA-1.
- The accepted conversion advanced the report from **1720 / 5925** to **1721 / 5924**, matched code from **76914 / 993856** to **76962 / 993856**, linked C units from **1260 / 5427** to **1261 / 5426**, and decomp files from **1461** to **1462**. The total-function denominator fell by one because the converted legacy local-label boundary is no longer inferred as a separate function.
- Evidence is retained in `.decomp-runs/round-170-isolation.json`, `.decomp-runs/round-170-08007E68-v3-source-audit.json`, `.decomp-runs/round-170-08007E68-apply.json`, `.nearmiss/func_08007E68.json`, `.decomp-runs/round-171-isolation.json`, `.decomp-runs/round-171-08007EAC-v1-source-audit.json`, `.decomp-runs/round-171-08007EAC-apply.json`, and `.decomp-runs/round-171-full-source-audit.json`.
...
- Docker full-section proofs matched `init_scheduled_function_task`'s 28-byte `.text` at SHA-256 `c2fa87380d2a8d57ae6b1ce372816ed3e46f7c509011814b233e106b6e8afd79` and `func_080D3A60`'s 28-byte linked `.text` at SHA-256 `1a2f62a3748d5b94e619e661e633e1f389e4ddb145dfd98b730b54df00116866`. Both normal isolation results were exact, so no metadata-only force was needed; the transactional Docker gate preserved ROM SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.
- The accepted pair advanced the report from **1711 / 5927** to **1713 / 5927**, matched code from **76610** to **76666**, linked C units from **1251 / 5436** to **1253 / 5434**, and decomp files from **1452** to **1454**. The full strict audit remains **0 instruction asm**, **106 files / 502 pins**, and **27 files / 31 barriers** including the **7 non-volatile** legacy findings.
...
## Round 160 — graphics-flag reset literal-pool boundary proof (2026-08-25)
- The existing typed `gGraphicsBuffer` bitfields gave `func_08006CC8` a strict ordinary-C spelling that reproduced every instruction. The normalized isolated candidate differed only because the target symbol ended at its local literal-pool label (24 bytes) while the candidate symbol covered 30 bytes.
- Linking both objects with the same absolute `gGraphicsBuffer` map proved equal complete `.text` sections at 32 bytes and equal SHA-256 `416cc334dbb73b34471b3b471474b35bd2428d0ed55b1abc970dfd199272eb4f`. The guarded force path remained metadata-only and the clean Docker ROM/report gate preserved SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.
- The accepted source passed strict audit with no asm, barrier, pin, non-mapped volatile, or opaque layout. Report progress advanced **1706 / 5932 → 1707 / 5931**, matched code **76476 → 76508**, linked C units **1246 → 1247**, and decomp files **1447 → 1448**.

## Round 161 — heap-cleanup wrapper and full-section symbol-boundary proof (2026-08-25)
- `func_08004378` was a strict ordinary-C named-record candidate: three pointer fields at the target's 0xC, 0x10, and 0x18 positions are freed in order, followed by the record. The accepted source has zero instruction asm, barriers, register pins, non-mapped volatile, and opaque layout findings.
- Normalized linked isolation stopped the target at an internal return label (26-byte target symbol versus 38-byte candidate symbol), although both complete `.text` sections were 40 bytes. A Docker compile plus `arm-none-eabi-objcopy -O binary -j .text` extraction produced equal target/candidate SHA-256 `2df6ce90e997e4751ec7b273acbecec807b9ed0294e0f6c97cebbfd76fb68c62`; `cmp` was exact. The guarded force path waived only the symbol-boundary metadata, and the full Docker gate preserved ROM SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.
- The accepted conversion advanced the report from **1707 / 5931** to **1708 / 5930** matched functions, code from **76508** to **76546**, linked C units from **1247** to **1248**, and decomp files from **1448** to **1449**. The denominator fell by one because the converted target's internal local-label boundary is no longer counted as a separate inferred function.
- The same screen also produced a strict named-record spelling for `func_08007FA4`: the input record's ID and the matched entry's field are forwarded to `func_08007E8C` in the target order, with zero source-quality findings. Its 22-byte target symbol versus 28-byte candidate symbol again required a complete `.text` proof; the 28-byte target/candidate SHA-256 is `6ceb9b46801a30727bbe8404dd3929367d9dedc5c17289b1504b4f8590e8f866`.
- The second accepted conversion advanced the report to **1709 / 5929**, matched code **76574**, linked C units **1249**, and decomp files **1450**. Both Round 161 wrappers demonstrate that the force path must remain restricted to complete-section byte equality plus the clean Docker ROM gate; the normalized symbol percentage alone is not sufficient.

## Round 162 — short-wrapper screen and canonical table-symbol proof (2026-08-25)
- The five-entry screen separated honest compiler near misses from a metadata-only candidate. `func_080020FC` reached an instruction-identical null-first body but still needed a complete-section proof; the scene-bit siblings `08016A60`/`08016A7C` retained real register/mask-home differences, and `08006EE0` hoisted the zero into a different register and changed the overlay's literal-pool shape. These remain evidence-only.
- `func_080047D4` reached an instruction-identical ordinary-C spelling after separating the table pointer from the integer index and keeping subtract-before-shift order. The target's 14-byte local-pool symbol versus the candidate's complete 20-byte symbol was proven by Docker-linking both sides with `D_083A49E8 = 0x083A49E8`; complete `.text` SHA-256 is `6db389b7381b9ff389dfe0368cb9226a19ca82d074650ce19cb86597f11551be`.
- The canonical ROM symbol was already in `include/undefined_syms.inc` but absent from `undefined_syms.ld`; adding the ordinary linker-map assignment was required for the full build. The accepted conversion advanced the report to **1710 / 5928**, matched code **76594 / 993844**, linked C units **1250 / 5437**, and decomp files **1451**. The clean Docker gate preserved ROM SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.

## Round 163 — nullable-record wrapper boundary proof (2026-08-25)
- The instruction-identical `func_080020FC` spelling used a named record and a null-first ordinary-C branch. Normalized isolation saw the target's 10-byte internal-label symbol versus the candidate's complete 16-byte symbol; Docker full-section extraction proved equal target/candidate SHA-256 `53786ccbc1988abd29d7171d20c8a030ab21d41fbca7ba9bb9069da97d6ccef4`.
- The guarded force path waived only that legacy symbol boundary. The accepted source audit remained strict-clean, and the full Docker gate advanced the report to **1711 / 5927**, matched code **76610 / 993844**, linked C units **1251 / 5436**, and decomp files **1452**, preserving ROM SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.
...
- The older `func_080059E4` ordinary-C spelling was byte-exact but strict-layout rejected because of three scalar byte-pointer offset operations; named variants retained real register-role mismatches.
- These functions were already C-linked with converted assembly/linker state, so source-only body replacement was the correct repository-state path. The full gate passed, and the accepted-source audit found zero instruction asm, barriers, compiler register pins, and non-mapped volatile accesses in all three files.
...
## Round 126 tooling follow-up — beatscript field pin/barrier cleanup (2026-08-23)
- The eight-entry screen found exact ordinary-C spellings for both fixed halfword field stores, the scaled word store, and the ready `func_080F0DE0` sibling. The selected three-function screen passed **3 exact / 0 rejected**; the intervening Round 125 included-scene mask screen remains near-miss evidence because candidate-only links expose target-TU padding/layout differences.
- `func_0800CAA4` and `func_0800CAB8` use separate ordinary base/offset locals; `func_0800D224` uses base/offset/field locals. All three are strict-policy clean and remove six pins plus four empty volatile-barrier expressions across three barrier-bearing files without changing the instruction streams. The tracked non-volatile barrier queue remains 11 files/11 barriers.
- These were already C-linked units, so the report remains **1704 / 5934** functions, **76392 / 993840** matched code, and **1244 C / 5443 asm-only** units. Source residue drops **180 → 177** pin-bearing files and **743 → 737** pins; the full strict audit now reports **38 files / 45 empty barrier findings**.
- The clean Docker build, Docker report, local `gen_objdiff.py`, and `decomp_cycle.py verify --no-report` all passed; the report was restored after verification. ROM/base ROM SHA-1 remains `3f556448d290fa5406d6ed367fee16cc02387ad3`.

## Round 128 tooling follow-up — nested-record and graphics-field cleanup (2026-08-23)
- Round 127 correctly rejected the first raw scalar-pointer spellings for the remaining graphics and nested-scene helpers. Named records and typed field addresses reduced the layout evidence to policy-clean forms; the Round 128 screen then found **4 exact candidate spellings / 4 near misses**, including exact variants for `func_0801D4A0`, `func_0805CB5C`, and `func_080F0DE0`.
- The final exact-only batch was **3 exact / 0 rejected**. The nested scene record for `func_0801D4A0` and named graphics field for `func_0805CB5C` are reusable ways to remove compiler pins without replacing incomplete layout knowledge with an opaque pointer blob.
- These were already C-linked units, so the report remains **1704 / 5934** functions, **76392 / 993840** matched code, and **1244 C / 5443 asm-only** units. Source residue drops **177 → 174** pin-bearing files, **737 → 731** pins, and the full audit drops **38 → 36** barrier-bearing files / **45 → 41** barrier findings.
- The clean Docker build, Docker report, local `gen_objdiff.py`, and `decomp_cycle.py verify --no-report` all passed; the report was restored after verification. ROM/base ROM SHA-1 remains `3f556448d290fa5406d6ed367fee16cc02387ad3`.

## Round 129 tooling follow-up — included-stub source-only pin cleanup (2026-08-23)
- The first screen attempt exposed a target-format trap: these three legacy host stubs already had their original `asm(...)` text moved to `asm/converted/`, so treating that file as standalone assembler produced target errors. Reusing the existing host object (`build/src/graphics_table.c.o`, `build/src/scenes/language_select.c.o`, or `build/src/scenes/soft_reset.c.o`) gave the correct byte baseline for isolated source screening.
- Eight policy-clean ordinary-C spellings produced **3 exact / 5 near miss** results. The exact table-walk and scene-wrapper forms were applied directly to the already-included `src/decomp` files, preserving the host TU and requiring no linker edit or assembly move. The full Docker gate is the integrated proof for this source-only path.
- The accepted three sources remove six compiler register pins and leave the report unchanged at **1704 / 5934** functions, **76392 / 993840** matched code, and **1244 C / 5443 asm-only** units. Full audit residue is now **171 pin-bearing files / 725 pins**, **36 barrier-bearing files / 41 findings**, and **0 instruction-bearing asm**.
- The ordinary absolute-table-base spelling for `func_080E1F48` remained a genuine near miss (`48.2` isolated gap); it was retained as rejected evidence rather than reintroducing the documented compiler barrier/pin. The clean Docker build, report refresh, local `gen_objdiff.py`, and repo verifier all passed with equal ROM/base ROM SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.

## Round 130 tooling follow-up — host-object boundary screening and three pin cleanups (2026-08-23)
- Host-object screening is the right baseline for already-converted included stubs. The selected `func_08012DCC`, `func_08014374`, and `func_080166AC` forms matched their target instruction streams, while candidate-only symbol boundaries surfaced harmless near-miss metadata; the integrated host-TU Docker build remained the authority.
- `func_08014374` needed the host TU's existing `extern void func_08015A88(void)` declaration. Named language/scene table overlays plus an ordinary `offset` local reproduced the target `ADDS R1,R1,R0` without compiler register pins. `func_08002514` remained a genuine register-home near miss and was not admitted.
- The accepted source audit is clean for all three files. The full audit now reports **168 pin-bearing files / 719 pins**, **36 barrier-bearing files / 41 findings**, and **0 instruction-bearing asm**. The report remains **1704 / 5934** functions, **76392 / 993840** code, and **1244 C / 5443 asm-only** units.
- The clean Docker build, report refresh, local `gen_objdiff.py`, and `decomp_cycle.py verify --no-report` all passed with equal ROM/base ROM SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.

## Round 131 tooling follow-up — named-overlay pin and barrier cleanup (2026-08-23)
- A fifteen-entry screen separated **7 exact / 4 near miss / 4 policy-rejected** ordinary-C candidates. `func_0801D4B4` became exact by reusing the named scene/data overlay pattern from its `0801D4A0` sibling; `func_080195E4` required named overlays for both non-encodable halfword offsets, while its raw scalar-pointer variants were correctly rejected by the layout gate.
- `func_080A002C` needs a widened `u32 value` assigned from the `u16` parameter, the player global loaded next, and an explicit `(u16)` normalization afterward. That source order reproduces the target's argument copy and `LSLS/LSRS` sequence without pinning R1.
- The accepted source audit is clean for all three files. The full audit now reports **165 pin-bearing files / 716 pins**, **35 barrier-bearing files / 39 findings**, and **0 instruction-bearing asm**. The report remains **1704 / 5934** functions, **76392 / 993840** code, and **1244 C / 5443 asm-only** units.
- The clean Docker build, report refresh, local `gen_objdiff.py`, and `decomp_cycle.py verify --no-report` all passed with equal ROM/base ROM SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.

## Round 132 tooling follow-up — host-TU mask-setter cleanup (2026-08-23)
- A six-entry host-object screen found instruction-identical ordinary-C bodies for `func_0800A3BC`, `func_080121B8`, and `func_08013114`. The candidate-only near misses were caused by the host TU's larger symbol/pool boundary, not body instructions; the full integrated Docker ROM gate confirmed the source-only replacements.
- Small named overlays plus the existing `value`/`mask = -mask` declaration order reproduce the target `LDRB; MOVS; RSBS; ANDS; STRB` sequence without compiler register homes. The same pattern is reusable for the remaining one-pin main-menu flag setters.
- The accepted source audit is clean for all three files. The full audit now reports **162 pin-bearing files / 713 pins**, **35 barrier-bearing files / 39 findings**, and **0 instruction-bearing asm**. The report remains **1704 / 5934** functions, **76392 / 993840** code, and **1244 C / 5443 asm-only** units.
- The clean Docker build, report refresh, local `gen_objdiff.py`, and `decomp_cycle.py verify --no-report` all passed with equal ROM/base ROM SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.

## Round 133 tooling follow-up — main-menu mask-wrapper cleanup (2026-08-23)
- Reusing the named scene-overlay and `value`/`mask` declaration order closed three adjacent `set_pause_beatscript_scene`/`scene_set_current_thread` mask wrappers: `func_080109CC`, `func_080144BC`, and `func_08014A0C`.
- The standalone `func_080109CC` candidate initially picked up an unrelated warning from the broad `src/beatscript.h` include; an explicit `extern void set_pause_beatscript_scene(u32)` preserved the body and made the candidate audit/build clean.
- Host-object screening reported only external-call relocations and candidate-only pool/symbol boundaries; the raw instruction bodies matched. The integrated host-TU Docker gate, rather than the isolated percentage, remained the acceptance proof.
- Full audit now reports **159 pin-bearing files / 710 pins**, **35 barrier-bearing files / 39 findings**, and **0 instruction-bearing asm**; report metrics remain **1704 / 5934**, **76392 / 993840**, and **1244 C / 5443 asm-only**. Docker build, report refresh, local `gen_objdiff.py`, and `decomp_cycle.py verify --no-report` all passed with equal ROM/base ROM SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.

## Round 134 tooling follow-up — main-menu callback-wrapper cleanup (2026-08-23)
- A five-entry host-object screen found the useful v1 shape for `func_08014878`, `func_08014C34`, and `func_08014C6C`: keep the address of `gCurrentSceneData` in an ordinary `struct Scene **base`, load the named `+0xDE` byte through `scene = *base`, then reload `*base` into a byte pointer for the callback offset.
- The explicit `offset = 0xBA`/`0xB8` followed by `offset <<= 1` and pointer add reproduced the target's `MOVS; LSLS; ADDS` callback-address sequence without a register pin. Modeling the large callback location as a named struct field instead caused a real three-register add/operand-home mismatch and remains rejected evidence.
- The selected v1 candidates were instruction-identical through their epilogues; candidate-only BL relocations and target pool/symbol-boundary tails explain the isolated near misses. The integrated host-TU ROM gate is required for these already-converted included stubs.
- Full audit now reports **156 pin-bearing files / 693 pins**, **35 barrier-bearing files / 39 findings**, and **0 instruction-bearing asm**; report metrics remain **1704 / 5934**, **76392 / 993840**, and **1244 C / 5443 asm-only**. Docker build, report refresh, local `gen_objdiff.py`, and `decomp_cycle.py verify --no-report` all passed with equal ROM/base ROM SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.

## Round 135 tooling follow-up — main-menu scene-init and callback siblings (2026-08-23)
- The ordinary scene overlay plus `D_03006518.unk1 = 0` spelling closed `func_080143F0`; the callback base/offset pattern closed `func_080148BC` without register homes.
- `func_08014490` needed a separate ordinary `u8 *data` local for the post-call `*base` reload. The named scene overlay alone, or reusing the same `scene` local, made agbcc select R1 for the reload; v3 reproduced the target `LDR R0,[R4]; STRB [R0,#8]` sequence.
- The five-entry host-object screen retained v1/v2 `08014490` register near-miss evidence and selected v3. All accepted bodies matched through their epilogues; isolated BL and pool/symbol-boundary differences were handled by the integrated host-TU ROM gate.
- Full audit now reports **153 pin-bearing files / 681 pins**, **35 barrier-bearing files / 39 findings**, and **0 instruction-bearing asm**; report metrics remain **1704 / 5934**, **76392 / 993840**, and **1244 C / 5443 asm-only**. Docker build, report refresh, local `gen_objdiff.py`, and `decomp_cycle.py verify --no-report` all passed with equal ROM/base ROM SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.

## Round 136 tooling follow-up — direct scene-init siblings (2026-08-23)
- The `func_080143F0` spelling generalized directly to `func_080119B8` and `func_08013428`: typed `D_03006518.unk1` stores, delayed scene overlay assignment, and ordinary `value`/`mask` locals preserved the target instruction bodies.
- `func_080143BC` needed the callback argument as a direct byte expression, `func_0801429C(data[0xFD], 1)`. Saving the byte into a widened `u32 value` caused an extra `MOV R0,R2`; the direct expression produced the target `LDRB R0; MOVS R1,#1` sequence.
- The five-entry host-object screen retained the widened-value `080143BC` spelling as a real register near miss and selected v2. The accepted three bodies matched through their epilogues; integrated host-TU verification handled only BL and pool/symbol-boundary metadata.
- Full audit now reports **150 pin-bearing files / 670 pins**, **35 barrier-bearing files / 39 findings**, and **0 instruction-bearing asm**; report metrics remain **1704 / 5934**, **76392 / 993840**, and **1244 C / 5443 asm-only**. Docker build, report refresh, local `gen_objdiff.py`, and `decomp_cycle.py verify --no-report` all passed with equal ROM/base ROM SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.

## Round 137 tooling follow-up — main-menu sibling and sprite-call cleanup (2026-08-23)
- `func_080133EC` matched with the recurring typed `D_03006518.unk1` store, named current-scene overlay, and ordinary `value`/`mask` declaration order. The integrated host-TU gate accepted the source-only included-stub replacement despite candidate-only boundary metadata.
- `func_080136A4` matched with the existing typed `sprite_set_anim_cel` interface and direct `gCurrentSceneSpritePool[6]` expression; this preserved the target `LDRSH` and call ABI without a typedef, pin, or barrier. The same named scene overlay handled the post-call mask.
- `func_0801197C` remains a genuine register-home near miss: ordinary locals changed the target `PUSH {R4,LR}` prologue and mask homes. Keep that candidate as evidence and do not force the conversion.
- The full audit now reports **148 pin-bearing files / 662 pins**, **35 barrier-bearing files / 39 findings**, and **0 instruction-bearing asm**. Report metrics remain **1704 / 5934**, **76392 / 993840**, and **1244 C / 5443 asm-only** units. Docker build, report refresh, local `gen_objdiff.py`, and `decomp_cycle.py verify --no-report` all passed with equal ROM/base ROM SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.

## Round 138 tooling follow-up — labeled table-helper loop cleanup (2026-08-23)
- The sibling helpers `func_08012768`, `func_08012798`, and `func_080127C8` all became exact with the same ordinary-C shape: `table`, shifted `index`, signed `value`, a labeled body before the `check` label, and a backward `if (value >= 0) goto body` edge. This preserved the target `R4` value lifetime across `func_0801274C` without pins.
- Generic `while`/structured-`if` spellings were policy-clean but real near misses: they inverted the branch layout or placed the call body after the check, adding branches and moving the literal pool. The failed screens remain evidence; the final v4 screen was **3 exact / 0 near miss**.
- The full audit now reports **145 pin-bearing files / 653 pins**, **35 barrier-bearing files / 39 findings**, and **0 instruction-bearing asm**. Report metrics remain **1704 / 5934**, **76392 / 993840**, and **1244 C / 5443 asm-only** units. Docker build, report refresh, local `gen_objdiff.py`, and `decomp_cycle.py verify --no-report` all passed with equal ROM/base ROM SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.

## Rounds 139–140 tooling follow-up — sibling table and graphics cleanup (2026-08-23)
- The `func_080127F8` sibling reused the body-before-check labeled loop from Round 138 and screened exact. `func_080126C8` reused the typed mode-store/scene-mask pattern; its isolated difference was only the known included-host BL and pool boundary, confirmed by the integrated gate.
- `func_080186AC` became ordinary C with a named multi-field graphics overlay. Its instruction body matched through the epilogue; the only isolated difference was the literal-pool representation of `0xDFFF`, not a register or semantic mismatch.
- The candidate policy correctly rejected raw scalar-pointer spellings for `func_0804E290` and `func_08082934` as opaque offset-heavy layouts. The `func_08011730` screen also showed a real argument/pointer register-home mismatch; retain it as evidence until a natural source shape preserves `R1` for the incoming value.
- The full audit now reports **142 pin-bearing files / 644 pins**, **35 barrier-bearing files / 39 findings**, and **0 instruction-bearing asm**. Report metrics remain **1704 / 5934**, **76392 / 993840**, and **1244 C / 5443 asm-only** units. Docker build, report refresh, local `gen_objdiff.py`, and `decomp_cycle.py verify --no-report` all passed with equal ROM/base ROM SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.

## Round 141 tooling follow-up — graphics-mask and scene-delta cleanup (2026-08-23)
- Named overlays closed `func_0801AE70` and `func_08039A44` without raw scalar-pointer aliases; their instruction bodies matched and only the literal-pool width differed in standalone screening.
- `func_080C4A48` followed the same named scene-variable field pattern with a signed `delta` and widened current halfword. Its body matched through the return; the isolated difference was the candidate/object pool boundary. Both legacy empty barriers were removed as ordinary C took over.
- `func_08003D28` remained a real register-home near miss: the natural bit/mask locals put the source bit in R2 and the mask in R1, opposite the target. Keep it evidence-only until a source shape naturally preserves the target roles.
- The full audit now reports **139 pin-bearing files / 635 pins**, **33 barrier-bearing files / 37 findings**, and **0 instruction-bearing asm**. Report metrics remain **1704 / 5934**, **76392 / 993840**, and **1244 C / 5443 asm-only** units. Docker build, report refresh, local `gen_objdiff.py`, and `decomp_cycle.py verify --no-report` all passed with equal ROM/base ROM SHA-1 `3f556448d290fa5406d6ed367fee16cc02387ad3`.

## Round 142 tooling follow-up — typed low-level helper cleanup (2026-08-23)
- The strict screen found exact ordinary-C spellings for `func_080029D0`, `func_080039D0`, and `func_080F2C68`. A byte/halfword union made the overlapping RMW accesses explicit for `080029D0`; a named four-byte record preserved the little-endian reader's four byte loads for `080039D0`; and ordinary `base`/`count`/`value` locals preserved the scan loop for `080F2C68`.
- The raw scalar-pointer spellings for the overlapping and allocation helpers were correctly policy-rejected, even where codegen was promising. `func_080B27B8` was instruction-identical but retained a candidate-only symbol-size/pool-tail difference; `func_080C69CC` had genuine register-home differences. Neither was forced into the accepted batch.
- The explicit Docker `make clean && make -j4` gate passed with `wariowareinc.gba: OK`, and the ROM SHA-1 stayed `3f556448d290fa5406d6ed367fee16cc02387ad3`. `decomp_cycle.py verify --no-report` exposed a tooling race in its `rm -rf build; make -j4` path: parallel compression rules can run before `build/graphics/title` is recreated, producing `FileNotFoundError`. The receipt is retained; the project make-clean path is the working workaround, followed by `make report` to restore generated artifacts.
- Full audit now reports **136 pin-bearing files / 627 pins**, **33 barrier-bearing files / 37 findings**, and **0 instruction-bearing asm**; report metrics remain **1704 / 5934**, **76392 / 993840**, and **1244 C / 5443 asm-only** units.

## Round 143 tooling follow-up — scene/table pool-boundary cleanup (2026-08-23)
- A seven-entry screen found three useful ordinary-C shapes. `func_080F0DFC` needed a pointer-typed table base plus a widened integer offset and separate mask local to preserve the target R1 base/R2 byte/mask homes. `func_08062488` and `func_080526D0` matched through their instruction bodies with named overlays.
- The isolated linked-ELF scores for the accepted three were metadata-only: `080F0DFC` and `080526D0` had target pool tails included in the function symbol, while `08062488` differed only in literal-pool width. The full Docker ROM gate and `decomp_cycle.py verify --no-report` both passed, confirming the linked ROM is the authoritative acceptance check for these standalone object boundaries.
- The nested arithmetic and scene-reload alternatives remained genuine register-home misses (`080F1B5C`, `080EC308`, `080DF440`) and were not forced. The strict accepted-source audit passed with no asm, barriers, pins, or non-mapped volatile accesses in the three replacements.
- Full audit now reports **133 pin-bearing files / 618 pins**, **33 barrier-bearing files / 37 findings**, and **0 instruction-bearing asm**; report metrics remain **1704 / 5934**, **76392 / 993840**, and **1244 C / 5443 asm-only** units. ROM/base ROM SHA-1 remains `3f556448d290fa5406d6ed367fee16cc02387ad3`.

## Round 144 tooling follow-up — named scene siblings and table-copy reloads (2026-08-23)
- The table/scene screen found one exact pin-free table cursor (`func_08003028`) and three instruction-identical named scene overlays (`func_080B27B8`, `func_080C6898`, and `func_080D28A4`). The scene candidates' isolated near-miss scores were target trailing zero/pool symbol metadata only; the integrated Docker ROM gate and verifier passed.
- The strict named-record candidates for `func_08002FC0` and `func_0800247C` exposed a real optimizer trap: agbcc reused the nonzero terminator value loaded by the check instead of emitting the target's redundant first-word reload in the copy body. Union and plain `register` experiments did not change that codegen; they remain rejected evidence rather than policy exceptions.
- This source-only batch required no linker or assembly move. The explicit clean Docker build emitted `wariowareinc.gba: OK`, `make report` regenerated 1244 C / 5443 asm-only units, and `decomp_cycle.py verify --no-report` passed after the clean build. Full audit residue is now **129 pin-bearing files / 604 pins**, **33 barrier-bearing files / 37 findings**, and **0 instruction-bearing asm**; ROM/base ROM SHA-1 remains `3f556448d290fa5406d6ed367fee16cc02387ad3`.

## Round 145 tooling follow-up — scene/global typed-array cleanup (2026-08-23)
- The ordinary-C screen found three useful pin-free shapes among four focused helpers. Named current-scene/data overlays closed `func_0806F0A0` and `func_0809E804`; direct typed arrays for the three canonical `D_03000E80`/`D_03000E88`/`D_03000E90` tables closed `func_080F2894`.
- `func_0800C9C0` remained a real register/order near miss even with named fields. For `080F2894`, pointer-local variants also changed the target load/store ordering; the direct typed-array spelling is the honest source shape. Isolated pool-tail or pool-width differences were accepted only after the integrated ROM gate.
- The first verifier invocation overlapped a second clean-build attempt and hit `rm: cannot remove 'build/src/decomp': Directory not empty`; after stopping the overlap and rerunning once, `decomp_cycle.py verify --no-report` passed with equal ROM/base ROM SHA-1. This reinforces that verifier/build calls must be serialized.
- The accepted source audit is clean for all three files. Full audit residue is now **126 pin-bearing files / 595 pins**, **33 barrier-bearing files / 37 findings**, and **0 instruction-bearing asm**. The report remains **1704 / 5934** functions, **76392 / 993840** code, and **1244 C / 5443 asm-only** units. Docker build, report refresh, local `gen_objdiff.py`, and verifier all passed.

## Round 146 tooling follow-up — graphics/table/scene pin cleanup (2026-08-23)
