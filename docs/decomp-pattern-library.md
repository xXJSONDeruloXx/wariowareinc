...
- **Scene-variable table lookup accumulator**: for a byte lookup through a pointer field at `gCurrentSceneVariable + 0x14`, load the scene-variable base and table pointer as separate locals, then write `table[arg1 * 0xE + arg0]`. `func_08030F9C` preserved the target's `LSLS #3; SUBS; LSLS #1; ADDS` index in R2 and the final `ADDS` into the table pointer.
