...
- **Narrow asm for generated-coordinate stack loads**: If C can match the frame and arg preservation but emits `LDRH; LSLS; ASRS` for a stack halfword or inserts a move before a helper call, a narrow `asm volatile` block can be used only for the call setup or indexed `LDRSH` loads. Keep the function body in C, expose outputs through register variables, and do not use a naked/whole-function wrapper. Example: `func_0800C15C` uses one narrow block for `func_08006F84` setup (`r9 = sp+0xA`) and one for the two `LDRSH` loads.
