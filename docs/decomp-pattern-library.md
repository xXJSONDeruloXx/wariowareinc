...
- **s16 callee-signature sign-extension trap**: When a function passes a sign-extended s16 value to a callee declared as `s32 sprite_is_invalid(void*, s16)`, the compiler adds extra `LSLS R1, #16; ASRS R1, #16` before the BL because it re-sign-extends the s16 argument per the callee's formal parameter type. The original asm doesn't have this — the value is already sign-extended in a register. **Mitigation**: Use naked inline asm for callers of `sprite_is_invalid` (and similar s16-parameter callees) where the original passes an already-sign-extended value. Example: `sprite_set_x`, `sprite_set_y` both needed naked asm due to this trap.
