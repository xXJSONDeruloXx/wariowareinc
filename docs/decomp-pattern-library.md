...
- **GBA BIOS SVC exception**: keep an irreducible BIOS call such as `func_080EE61C` as a standalone source in `asm/`, not as non-empty inline asm in `src/decomp`. Its semantics are signed division, but bundled agbcc lowers C `/` to `__divsi3` and has no SVC/SWI builtin; exact bytes require the two-instruction assembly body.
