...
function buildChunkPrompt(chunk) {
  return [
    ...
    "",
    "## Step 4b — If the chunk has room, convert an existing naked asm file to real C",
    "This is optional and should not block the primary chunk goal. Only do it after a normal candidate is done or when the candidate is blocked, and only if the function looks like a genuine C-shaping problem rather than a known hard case.",
    "Use the documented effort checklist: try pure C, register pinning, asm volatile barriers/clobbers, statement reordering, type shaping, load-base-first, pointer shaping, and goto loops before resorting to naked asm.",
    "Prefer functions that were only left naked because of register allocation or instruction-order quirks; skip the known hard cases that truly require asm.",
    "",
