...
def main() -> int:
    ...
    if json_path.is_file():
        try:
            old_score = float(json.loads(json_path.read_text()).get("score"))
        except (OSError, TypeError, ValueError, json.JSONDecodeError):
            pass

    attempts = root / "tools" / "attempts.tsv"
    if not attempts.exists():
        attempts.write_text("timestamp\tfunc\tfile\tscore\treason\tcommit\tcandidate\n")
    ...
    if old_score is not None and old_score <= args.score:
